...
export default class extends think.Controller {
    ...
    async indexAction() {
        try {
            ...
            const next_article = await think.model('article').cache(env + `article_no${article_no}_next`, {timeout: 2 * 60 * 1000}).where({del: 0, category_id: result.category_id, article_id: ['<', result.article_id]}).find();
            const prev_article = await think.model('article').cache(env + `article_no${article_no}_prev`, {timeout: 2 * 60 * 1000}).order('created_at DESC').where({del: 0, category_id: result.category_id, article_id: ['>', result.article_id]}).group('article_id').find();
