...
export default class extends think.Model {
    async goodsList($data: GoodsListParams) {
        ...
        if($data.status == -1) {
            ...
            // @ts-ignore
            return this.setRelation(false).field('sale_num,created_at,updated_at,status,shop_id,id,name,category_id,old_price,current_price,weight,sum_stock,min_buy,max_buy,images,thumb_image_path,desc').order({created_at: 'DESC'}).where(where).page(page, limit).countSelect();
        } else {
    ...
    async getGoodById($id: number,shop_id: number) {
        ...
        return this.field('shop_id,id,name,category_id,old_price,current_price,weight,sum_stock,min_buy,max_buy,images,thumb_image_path,sku_list,sku_show,detail,desc').where({id: $id,shop_id}).find();
    }
